...
export default function Depositors() {
  const { data: depositors = [], isLoading, isError } = useGetDepositorsQuery();
  const [createDepositor, { isLoading: creating }] = useCreateDepositorMutation();
  const [updateDepositor, { isLoading: updating }] = useUpdateDepositorMutation();
            ...
            <tbody className="divide-y divide-navy-100">
              {filtered.map((d) => (
                <tr key={d._id} className="hover:bg-navy-50/50">
                  <td className="px-4 py-3 text-navy-900">{d.firstName}</td>
                  <td className="px-4 py-3 text-navy-900">{d.lastName}</td>
